 ...
 * @Copyright
 */
public class FileTypeUtil extends BaseLogger {

	private static FileTypeUtil fileTypeUtil = null;

	public synchronized static FileTypeUtil getInstance() {
		if (fileTypeUtil == null) {
			fileTypeUtil = new FileTypeUtil();
		}
		return fileTypeUtil;
	}
	...
	public boolean isAudio(String fileName) {
		String reg = "\\.(flac|aif|wav|au|mp3|ram|wma|mmf|amr|aac)$";
	...
	public boolean isVideo(String fileName) {
		String reg = "\\.(mp4|3gp|flv|avi|mov|wmv9|rm|rmvb|wmv|asx|asf|divx|mpg|mpeg|mpe|mkv|vob)$";
		Pattern p = Pattern.compile(reg);
		boolean boo = p.matcher(fileName.toLowerCase()).find();
		return boo;
	}
	...
	public boolean isImage(String fileName) {
		String reg = "\\.(bmp|dib|gif|jfif|jpe|jpeg|jpg|png|tif|tiff|ico)$";
		Pattern p = Pattern.compile(reg);
		boolean boo = p.matcher(fileName.toLowerCase()).find();
		return boo;
	}
	...
	public static void main_test(String[] args) {
		...
		String type = fileNameMap.getContentTypeFor("E://Kalimba.mp3");
		System.out.println(type);

		System.out.println(getInstance().isAudioFileType("Kalimba.mp3"));
	}
}
