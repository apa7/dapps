...
public class ThumbnailatorUtils {  
    ...
    /**
     * @Description: 图片尺寸不变，压缩图片文件大小outputQuality实现,参数1为最高质量
     * @Datatime 2017年8月6日 下午2:24:53 
     * @return void    返回类型
     */
    public static void outputQuality(File formSource,Float scale,File toSource) {
    	 try {
			Thumbnails.of(formSource).scale(1f).outputQuality(scale).outputFormat("jpg").toFile(toSource);
		} catch (IOException e) {
