...
public class ObjectUtils {
	...
	public static String checkFieldValueNull(Object bean) throws Exception {
		...
		if (bean == null) {
			...
			for (Field f : bean.getClass().getDeclaredFields()) {
				...
				if (ObjectUtils.isBlank(f.get(bean))) { // 判断字段是否为空，并且对象属性中的基本都会转为对象类型来判断
					message = f.getName();
					break;
				}
			}
	...
	public static String checkFieldsValueNull(Object bean,
		...
		for (Field f : bean.getClass().getDeclaredFields()) {
			...
			if (!ObjectUtils.isObjectsBlank(checkModel, filedNames)) {
				boolean flag = false;
				switch (checkModel) {
				case BESIDES:
				...
				if (ObjectUtils.isNotBlank(message)) {
					break;
				}
			}
		}
	 ...
	 * @throws Exception
	 */
	public static String checkFieldValueNullBesides(Object bean,String... filedNames) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static String checkFieldValueNullContains(Object bean,String... filedNames) throws Exception {
